...
Replace the "hidden on mobile" sidebar with a slide-in overlay triggered by a hamburger button in the navbar. Use Alpine.js for toggle state. Add backdrop overlay. Close on navigation, escape key, and backdrop click. Include focus trap and screen reader dialog semantics.
...
- [ ] Slide transition respects `prefers-reduced-motion` (instant show/hide if set)

## Files to modify

- `src/hyperadmin/templates/_navbar.html` -- add hamburger button (`data-testid="sidebar-toggle"`)
- `src/hyperadmin/templates/_sidebar.html` -- Alpine.js open/close state, backdrop div, role="dialog", aria-label
...
- Focus trap: use Alpine.js `x-trap` or manual focus loop on Tab/Shift+Tab
- `@media (prefers-reduced-motion: reduce)`: disable slide transition, use instant show/hide
...
4. Tab through sidebar links -- focus stays trapped
5. Open screen reader -- sidebar announces as "Navigation" dialog
