...
def test_create_form_view_renders_form(client: TestClient):
    ...
    response = client.get("/admin/testproduct/create")
    assert response.status_code == 200
    assert "<form" in response.text
    assert 'hx-post="http://testserver/admin/testproduct"' in response.text
...
def test_create_view_successful_submission(client: TestClient):
    ...
    assert response.headers["HX-Redirect"] == "http://testserver/admin/testproduct/1"

    # Verify the item was created in the database
    get_response = client.get("/admin/testproduct/1")
